...
def get_random_proxy():
    """Get a random proxy from webshare.io like in testehraz.py"""
    import random
    random_user = f"jjebraham-{random.randint(1, 100)}"
    return f"http://{random_user}:Amir1234@p.webshare.io:80"
...
async def _ehraz_post(url: str, payload: dict, timeout_seconds: int = 15):
    ...
    # Try proxy first (since direct connection is often blocked)
    for attempt in range(3):
        proxy_url = get_random_proxy()
        try:
                ...
                async with session.post(
                    ...
                    if resp.status == 200:
                        ...
                        # Format 2: {"code": "0", "MessageFromCore": {...}}
                            ...
                            # If code is "0" but MessageFromCore has error, treat as not matched
                            message_core = data.get("MessageFromCore", {})
                            if message_core.get("code") == "card.not_valid":
                                logger.warning(f"EHRAZ card validation failed: {message_core.get('message')}")
                                return {"matched": False, "error": message_core.get("message")}
                            else:
                                # Other MessageFromCore codes might indicate success
                                logger.info(f"EHRAZ MessageFromCore: {message_core}")
                                return {"matched": True}
                        # Unknown format
                        else:
                            logger.warning(f"EHRAZ unknown response format: {data}")
                            return {"matched": False, "error": "unknown_response_format"}
                    else:
                        logger.warning(f"EHRAZ proxy failed with status: {resp.status}")
                        text = await resp.text()
                        logger.warning(f"EHRAZ proxy response: {text}")
        except Exception as e:
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                json=payload,
                headers=headers,
                timeout=aiohttp.ClientTimeout(total=5),  # Shorter timeout for direct
            ) as resp:
                if resp.status == 200:
                    data = await resp.json()
                    logger.info(f"EHRAZ direct success: {data}")
                    # Handle response formats as above
                    if "matched" in data:
                        return data
                    elif "code" in data and "MessageFromCore" in data:
                        message_core = data.get("MessageFromCore", {})
    ...
    return {"matched": False, "error": "all_attempts_failed"}
...
async def register_user(req: RegisterRequest):
    ...
    # Verify phone number matches national ID using EHRAZ API
    logger.info(f"Verifying phone {req.phone_number} matches national ID {req.national_id}")
    try:
        # Call EHRAZ API directly for mobile verification
        mobile_verification_data = await _ehraz_post(
            "https://ehraz.io/api/v1/match/national-with-mobile",
            {
                "nationalCode": req.national_id,
                "mobileNumber": req.phone_number,
            },
            timeout_seconds=15,
        )
        
        mobile_matched = bool(mobile_verification_data.get("matched", False))
        logger.info(f"EHRAZ mobile verification result: {mobile_matched}")
        
        if not mobile_matched:
            raise HTTPException(
                status_code=400,
                detail="phone_national_id_mismatch",
            )
    except HTTPException:
        # Re-raise HTTP exceptions
        raise
    except Exception as e:
        logger.error(f"EHRAZ mobile verification failed: {e}")
        # If EHRAZ verification fails, we could either:
        # 1. Allow registration anyway (current behavior)
        # 2. Block registration (more secure)
        # For now, we'll log the error but allow registration
        # to avoid blocking users if EHRAZ API is down
...
async def verify_with_ehraz(req: EhrazRequest):
    ...
    if error_msg:
        logger.info(f"EHRAZ verification result: {result}, error: {error_msg}")
    else:
        logger.info(f"EHRAZ verification result: {result}")
    return {"matched": result, "error": error_msg}
