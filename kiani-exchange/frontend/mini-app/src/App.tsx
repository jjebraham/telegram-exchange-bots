...
import { Users, ArrowLeftRight, Shield, Activity, Bell, Settings, TrendingUp, DollarSign, AlertCircle, CheckCircle, XCircle, Clock, Search, Filter, Download, RefreshCw, Menu, X, MessageSquare, BarChart3, FileText, Wallet, Gift, AlertTriangle, Code, Zap, Database, Server } from 'lucide-react';
...
const ApiManagementComponent = () => {
  ...
  const createApiKey = async () => {
    ...
    try {
      // TODO: Implement actual Supabase Edge Function call
      alert('API Key creation would call: https://lavhlalqzbkuqjyukgbk.supabase.co/functions/v1/admin-create-api-key');
      setNewKeyName('');
      fetchApiKeys();
    } catch (error) {
      console.error('Error creating API key:', error);
      alert('Error creating API key. Check console for details.');
    } finally {
      setCreating(false);
    }
  };
  ...
  return (
    <div className="p-6">
      ...
      <div className="bg-gray-800/50 border border-gray-700/50 rounded-xl p-4 mb-6">
        <h3 className="font-bold mb-4 text-white">API Keys</h3>
        ...
        <div className="mb-4">
          <input
            ...
            className="bg-gray-700 border border-gray-600 rounded px-3 py-2 text-white w-full max-w-md"
          />
          <button
        ...
        <div className="space-y-2">
          {loading ? (
            ...
            apiKeys.map((key) => (
              <div key={key.id} className="border border-gray-700/50 rounded p-3">
                <div className="flex justify-between items-center">
                  <span className="font-medium">{key.name}</span>
                  <span className="text-sm text-gray-400">{key.created_at}</span>
                </div>
            ...
            <div className="text-gray-400">
              No API keys found. Create one above or check Supabase setup.
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-gray-800/50 border border-gray-700/50 rounded-xl p-4">
        <h3 className="font-bold mb-4 text-white">External API Configuration</h3>
        <div className="text-gray-400">
          Configure external APIs like EHRAZ, Wallex, etc.
          <br />
          <br />
          <strong>Setup Required:</strong>
          <ol className="list-decimal pl-5 mt-2 space-y-1">
            <li>Run database migration in Supabase dashboard</li>
            <li>Deploy Edge Functions</li>
            <li>Create admin user with superadmin role</li>
...
function RegistrationPage({
  ...
  const updateField = useCallback(
    (field: keyof RegistrationFormData, value: string | boolean) => {
      // Keep name/surname text exactly as the user types it.
      // Normalization/trim happens only when submitting the form.
      // This allows multi-word Persian names such as:
...
function AdminPanelPage() {
  ...
  const navigationItems = [
    ...
    { id: 'wallets', name: 'Wallet Management', icon: Wallet, category: 'finance' },
    { id: 'rates', name: 'Exchange Rates', icon: TrendingUp, category: 'operations' },
    { id: 'fees', name: 'Fee Configuration', icon: DollarSign, category: 'finance' },
    { id: 'limits', name: 'Transaction Limits', icon: AlertCircle, category: 'compliance' },
    { id: 'broadcast', name: 'Broadcast Message', icon: MessageSquare, category: 'communication' },
    { id: 'support', name: 'Support Tickets', icon: MessageSquare, category: 'communication' },
    { id: 'referrals', name: 'Referral Program', icon: Users, category: 'marketing' },
    { id: 'promotions', name: 'Promotions & Rewards', icon: Gift, category: 'marketing' },
    { id: 'disputes', name: 'Dispute Resolution', icon: AlertTriangle, category: 'operations' },
    { id: 'transactions', name: 'Transaction Logs', icon: FileText, category: 'reports' },
    { id: 'audit', name: 'Audit Trail', icon: Shield, category: 'compliance' },
    { id: 'reports', name: 'Financial Reports', icon: FileText, category: 'reports' },
    { id: 'analytics', name: 'Analytics Dashboard', icon: Activity, category: 'reports' },
    { id: 'security', name: 'Security Logs', icon: Shield, category: 'compliance' },
    { id: 'api', name: 'API Management', icon: Code, category: 'developer' },
    { id: 'webhooks', name: 'Webhooks', icon: Zap, category: 'developer' },
    { id: 'backup', name: 'Backup & Recovery', icon: Database, category: 'system' },
    { id: 'maintenance', name: 'System Maintenance', icon: Server, category: 'system' },
    { id: 'notifications', name: 'Push Notifications', icon: Bell, category: 'communication' },
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-700/30">
              {filteredUsers.map((u) => <tr key={u.id} className="hover:bg-gray-900/30">
                ...
                <td className="px-4 py-3">{u.kyc_status === 'Approved' && Number(u.verification_level || 1) >= 2 ? <span className="px-2 py-1 rounded bg-green-500/20 text-green-400 border border-green-500/30">Verified</span> : <span className="px-2 py-1 rounded bg-yellow-500/20 text-yellow-400 border border-yellow-500/30">Pending</span>}</td>
                <td className="px-4 py-3"><div className="flex items-center gap-2"><button
...
>
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'disputes': return <div className="text-white text-center py-20">Dispute Resolution Coming Soon...</div>;
      ...
      case 'audit': return <div className="text-white text-center py-20">Audit Trail Coming Soon...</div>;
      case 'reports': return <div className="text-white text-center py-20">Financial Reports Coming Soon...</div>;
      case 'analytics': return <div className="text-white text-center py-20">Analytics Dashboard Coming Soon...</div>;
      case 'security': return <div className="text-white text-center py-20">Security Logs Coming Soon...</div>;
      case 'api': return <ApiManagementComponent />;
      case 'webhooks': return <div className="text-white text-center py-20">Webhooks Coming Soon...</div>;
      case 'backup': return <div className="text-white text-center py-20">Backup & Recovery Coming Soon...</div>;
      case 'maintenance': return <div className="text-white text-center py-20">System Maintenance Coming Soon...</div>;
      case 'notifications': return <div className="text-white text-center py-20">Push Notifications Coming Soon...</div>;
      ...
      <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-gray-900/95 backdrop-blur-xl border-r border-gray-700/50 transform transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="p-4 border-b border-gray-700/50">
          <select
            ...
            className="w-full px-3 py-2 bg-gray-800/50 border border-gray-700/50 rounded-lg text-white text-sm focus:border-blue-500/50 focus:outline-none"
          >
            {categories.map((cat) => (
