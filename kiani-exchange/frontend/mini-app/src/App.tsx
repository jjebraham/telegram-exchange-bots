  ...
  const renderDashboard = () => (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <StatCard title="24h Volume" value={report?.total_send_amount ? `${Number(report.total_send_amount).toLocaleString()} ` : '-'} icon={<DollarSign className="w-8 h-8 text-purple-400" />} color="purple" />
            ...
            <div key={req.reference_number} className="flex items-center justify-between p-4 bg-gray-900/50 rounded-lg border border-gray-700/30">
              <div className="flex items-center space-x-4 space-x-reverse">
                ...
                <div>
                  ...
                  <p className="text-gray-400 text-sm">{req.exchange_pair} • {Number(req.send_amount).toLocaleString()}</p>
                </div>
              </div>
      ...
      <div className="bg-gray-800/50 border border-gray-700/50 rounded-xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-700/30">{transactions.map((req)=><tr key={req.reference_number} className="hover:bg-gray-900/30"><td className="px-4 py-3 text-gray-300 font-mono">#{req.reference_number}</td><td className="px-4 py-3 text-white">{req.user_name}</td><td className="px-4 py-3 text-gray-300">{req.exchange_pair}</td><td className="px-4 py-3 text-gray-300">{Number(req.send_amount).toLocaleString()}</td><td className="px-4 py-3 text-gray-300">{req.timestamp}</td><td className="px-4 py-3"><span className={`px-2 py-1 rounded-full text-xs border ${getStatusColor(req.status)}`}>{req.status}</span></td></tr>)}</tbody>
          </table>
        </div>
      </div>
